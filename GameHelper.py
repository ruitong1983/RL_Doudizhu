...
def DrawRectWithText(image, rect, text):
    print("DrawRectWithText-image:", np.asarray(image))
    ...
    def Screenshot(self, region=None):  # -> (im, (left, top))
        ...
        while try_count > 0 and not success:
            try:
                ...
                # im = Image.open(r"C:\Users\RuiTong\Downloads\11111.png")
                ...
                # for hWndItem in hWnd_list:
                #
                #     if win32gui.GetWindowText(hWndItem) == "腾讯手游助手(64位)":
                #         self.Handle = hWndItem
                #         print(hWndItem, win32gui.GetWindowText(hWndItem))
                self.Handle = win32gui.FindWindow(None, u'腾讯手游助手(32位)')
                # self.Handle = win32gui.FindWindow('WeChatMainWndForPC', None)
                hwnd = self.Handle
                print("hwnd==>",hwnd)
                left, top, right, bot = win32gui.GetWindowRect(hwnd)
                print("GetWindowRect==>", left, top, right, bot)
                width = right - left
                height = bot - top
